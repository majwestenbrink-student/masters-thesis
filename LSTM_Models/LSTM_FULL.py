"""
Rolling LSTM — Full Lag Features (row-based)
=============================================
Uses pre-computed lag columns: LAG{L}_PC1-4, RMSE_LAG{L} for
L in [211,190,...,22] plus CURRENT_PC1-4, RMSE_CURRENT.

All information is contained within each row — no column shifting.

Architecture: LSTM(50) → LSTM(20) → Dense(4, tanh)
Sequence shape: (11 timesteps, 5 features)
  Timesteps ordered oldest→newest:
    lag211, lag190, lag169, lag148, lag127, lag106,
    lag85, lag64, lag43, lag22, current
  Features per timestep: [PC1, PC2, PC3, PC4, RMSE]

Input file : /home/cdsw/LSTM_FULL_Upload.csv
Checkpoints: /home/cdsw/checkpoints-LSTM_FULL_Upload/
Output     : /home/cdsw/LSTM_DONE/LSTM_FULL_Done.csv
...
def worker_fn(worker_id, indices, numpy_data, skip_set, out_path, params):
    ...
    def build_sequences(start, end):
        ...
        for j in range(start, end):
            seq = seq_arr[j]                        # (N_TIMESTEPS, N_SEQ_FEAT)
    ...
    def build_single(i):
        seq = seq_arr[i]                            # (N_TIMESTEPS, N_SEQ_FEAT)
        if np.any(np.isnan(seq)):
            return None
        return seq[np.newaxis]                      # (1, N_TIMESTEPS, N_SEQ_FEAT)
...
if __name__ == '__main__':
    ...
    print('='*70)
    print('Rolling LSTM — Full Lag Features (row-based)')
    ...
    missing = [c for c in ALL_SEQ_COLS + TARGET_COLS if c not in df.columns]
    if missing:
        raise ValueError(f'Missing columns: {missing}')
    print(f'  All {len(ALL_SEQ_COLS)} sequence columns confirmed.')
    ...
    # Checkpoints
    ckpt_main     = os.path.join(CKPT_DIR, 'lstm_full_checkpoint.csv')
    partial_paths = [os.path.join(CKPT_DIR, f'lstm_full_partial_w{w}.csv')
