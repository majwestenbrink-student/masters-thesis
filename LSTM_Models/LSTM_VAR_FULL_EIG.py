"""
Rolling LSTM — VAR Lags + Full Lag Features + Eigenvalues (NO IV)
==================================================================
Single-branch architecture — all features share same 11 sparse timesteps:
  combined (11, 9): [PC1..4, RMSE, VAR_PC1..4] → LSTM(50) → LSTM(20) ──┐
...
Feature breakdown per timestep:
  ...
  (IV excluded to isolate its importance)

Input file : /home/cdsw/LSTM_VAR_FULL_EIG_Upload.csv
Checkpoints: /home/cdsw/checkpoints-LSTM_VAR_FULL_EIG_Upload/
Output     : /home/cdsw/LSTM_DONE/LSTM_VAR_FULL_EIG_NO_IV_Done.csv
...
# ── Configuration ──────────────────────────────────────────────────────────
CSV_NAME  = 'LSTM_VAR_FULL_EIG_Upload'
FILE_PATH = f'/home/cdsw/{CSV_NAME}.csv'
CKPT_DIR  = f'/home/cdsw/checkpoints-{CSV_NAME}'
DONE_DIR  = '/home/cdsw/LSTM_DONE'
DONE_PATH = f'{DONE_DIR}/LSTM_VAR_FULL_EIG_NO_IV_Done.csv'
LOG_PATH  = f'{CKPT_DIR}/lstm_var_full_eig_no_iv_log.txt'
...
N_SEQ_FEAT   = 9                          # PC1..4 + RMSE + VAR_PC1..4 (no IV)
...
if __name__ == '__main__':
    ...
    print('='*70)
    print('Rolling LSTM — VAR Lags + Full Lag Features + Eigenvalues (NO IV)')
    print(f'  File       : {FILE_PATH}')
    print(f'  Checkpoints: {CKPT_DIR}')
    print(f'  Output     : {DONE_PATH}')
    print(f'  Train win  : {TRAIN_WINDOW}  |  Horizon : {HORIZON}')
    print(f'  Timesteps  : {ALL_STEPS}')
    print(f'  Combined   : ({N_TIMESTEPS}, {N_SEQ_FEAT})  [PC1-4+RMSE | VAR_PC1-4]')
    ...
    print(f'  All {len(ALL_SEQ_COLS)} sequence columns, {len(STATIC_COLS)} static confirmed. (IV excluded)')
    ...
    # Combine into single array: (N, 11, 9)
    combined_arr = np.concatenate([seq_arr, var_arr], axis=2).astype(np.float32)
    print(f'  combined_arr shape: {combined_arr.shape}  (seq:5 + var:4)')
    ...
    # Checkpoints
    ckpt_main     = os.path.join(CKPT_DIR, 'lstm_var_full_eig_no_iv_checkpoint.csv')
    partial_paths = [os.path.join(CKPT_DIR, f'lstm_var_full_eig_no_iv_partial_w{w}.csv')
